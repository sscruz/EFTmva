...
def main():
    ...
    sm,bins,_  = plt.hist(param_score, weights=sm_weight.detach() , bins=200 , alpha=0.5, label="SM" , density=True)
    bsm,bins,_ = plt.hist(param_score, weights=bsm_weight.detach(), bins=bins, alpha=0.5, label="BSM", density=True)
    plt.legend()
    ...
    sm,bins,_  = plt.hist(dedicated_score, weights=sm_weight.detach() , bins=200 , alpha=0.5, label="SM" , density=True)
    bsm,bins,_ = plt.hist(dedicated_score, weights=bsm_weight.detach(), bins=bins, alpha=0.5, label="BSM", density=True)
    plt.legend()
